...
def fetch_tgn_data(tgn_id: str, record_id):
    if "/" in tgn_id:
        raise
    ...
    try:
        ...
        if parent:
            ...
            part_data = fetch_tgn_data(data["part_of"], None)
        ...
        for row in reader:
            ...
            if same_as:
                if same_as not in cache:
                    temp = fetch_tgn_data(same_as, pi_record_no)
                    for t in temp:
                        cache[t["tgn_id"]] = t
            ...
            if part_of:
                if part_of not in cache:
                    temp = fetch_tgn_data(part_of, pi_record_no)
                    for t in temp:
                        cache[t["tgn_id"]] = t
            ...
            # except csv.Error or UnicodeDecodeError:
            #     traceback.print_exc()

            #     with open("goup_tgn.txt", "a") as f:
            #         f.write(str(pi_record_no))
            #         f.write('\n')
            #         f.write(traceback.format_exc())
            #         f.write('\n\n\n')
            ...
            with open("./data/goupil/goupil_tgn_1.json", "w") as f:
                json.dump(data, f, indent=4, sort_keys=True, ensure_ascii=False)

            with open("./data/goupil/tgn_1.json", "w") as f:
