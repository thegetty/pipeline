...
"""
This script converts a CSV export of in the format of 'collapsed_sales-descriptions-places.csv'
can be used as a service file in the provenance pipelines.

    python ./generate_sales_description_tgn_places.py
"""
...
if __name__ == "__main__":
    ...
    with open(filename, "r", encoding="utf-8-sig") as csv_file:
        ...
        for row in reader:
            ...
            if catalog_number not in data:
                data[catalog_number] = {}

            if field not in data[catalog_number]:
                data[catalog_number][field] = {}

            assert place not in data[catalog_number][field]
            data[catalog_number][field][place] = tgn_data

    with open("../data/sales/sales_description_tgn.json", "w") as f:
        json.dump(data, f, indent=4, sort_keys=True, ensure_ascii=False)

    with open("../data/sales/tgn.json", "w") as f:
