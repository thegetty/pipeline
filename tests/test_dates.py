...
import pipeline.nodes.cleaners
...
class TestDateCleaners(unittest.TestCase):
	...
	def test_date_cleaner(self):
		'''
		Test the documented formats that `pipeline.nodes.cleaners.date_cleaner` can parse
		...
		for value, expected in tests.items():
			date_range = pipeline.nodes.cleaners.date_cleaner(value)
