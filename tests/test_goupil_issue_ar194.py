...
class PIRModelingTest_AR194(TestGoupilPipelineOutput):
    ...
    def test_modeling_ar194_2(self):
        ...
        self.assertEqual(
            agents_activity["carried_out_by"][0]["id"],
            "tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:shared#PERSON,AUTH,Petit%2C%20Georges",
        )
        acquisition = out_with_agents["part"][1]
        self.assertTrue(
            {"transferred_title_from", "transferred_title_of", "transferred_title_to", "part"}.issubset(
                set(acquisition.keys())
            )
        )
        agents_activity = payment["part"][0]
        self.assertEqual(agents_activity["type"], "Activity")
