...
class PIRModelingTest_PrivateContractSales(TestProvenancePipelineOutput):
	...
	def verify_catalogs(self, output):
		...
		expected_event_id = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#LOTTERY-EVENT,CATALOGNUMBER,D-A50'
	...
	def verify_sales(self, output):
		...
		* An Activity modeling the individual private sale
		...
		hmo_key = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#OBJECT,D-A50,0001,1765'
		hmo = objects[hmo_key]
		
		sale_curr = drawings['tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#AUCTION,D-A50,LOT,0001,DATE,1765']
		
		event_key = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#LOTTERY-EVENT,CATALOGNUMBER,D-A50'
		sale_event = activities[event_key]
		
		object_set_key = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#AUCTION,D-A50,LOT,0001,DATE,1765-Set'
