...
class PIRModelingTest_PrivateContractSales(TestProvenancePipelineOutput):
	...
	def verify_catalogs(self, output):
		...
		pvt_sale = activities['tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#PRIVATE_CONTRACT_SALE-EVENT,CATALOGNUMBER,B-267']
	...
	def verify_sales(self, output):
		...
		* A Provenance Entry for the seller's previous ownership, including:
		...
		hmo_key = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#OBJECT,B-267,0001,1817'
		hmo = objects[hmo_key]
		
		prov_entry_curr = activities['tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#AUCTION-TX,B-267,1817,0001']
		prov_entry_prev = activities['tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#OBJECT,B-267,0001,1817-seller-0-ProvenanceEntry']
		
		event_key = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#PRIVATE_CONTRACT_SALE-EVENT,CATALOGNUMBER,B-267'
		sale_event = activities[event_key]
		
		object_set_key = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#AUCTION,B-267,LOT,0001,DATE,1817-Set'
		...
		event_organizer = 'tag:getty.edu,2019:digital:pipeline:REPLACE-WITH-UUID:provenance#AUCTION-HOUSE,PI_REC_NO,22949,0' # TODO this should be pulled from the data
