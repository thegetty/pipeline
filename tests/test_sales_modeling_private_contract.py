...
class PIRModelingTest_PrivateContractSales(TestSalesPipelineOutput):
    ...
    def verify_sales(self, output):
        ...
        * A Provenance Entry for the seller's previous ownership, including:
        ...
        self.assertEqual(len(prices), 1)
        self.assertEqual({c['_label'] for c in prices[0]['classified_as']}, {'Sale Price'})
        ...
        self.assertEqual(sorted([p['type'] for p in prov_entry_curr['part']]), ['Acquisition', 'AttributeAssignment', 'Payment', 'Payment', 'TransferOfCustody', 'TransferOfCustody'])
